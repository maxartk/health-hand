...
@JsonClass(generateAdapter = true) data class AutomationCounts(val pending:Int=0,val delivered:Int=0,val failed:Int=0)
@JsonClass(generateAdapter = true) data class AutomationSummaryResponse(val ok:Boolean=false,val error:String?=null,val configured:Boolean=false,val counts:AutomationCounts=AutomationCounts(),val last_success_at:Long?=null,val last_failure_at:Long?=null,val last_error:String?=null)
@JsonClass(generateAdapter = true) data class AutomationEvent(val event_id:String="",val event_name:String="",val summary:String="",val status:String="pending",val attempts:Int=0,val last_error:String?=null,val created_at:Long=0,val updated_at:Long=0,val delivered_at:Long?=null,val next_attempt_at:Long=0)
@JsonClass(generateAdapter = true) data class AutomationEventsResponse(val ok:Boolean=false,val error:String?=null,val events:List<AutomationEvent> = emptyList())
@JsonClass(generateAdapter = true) data class AutomationRetryRequest(val event_id:String)
@JsonClass(generateAdapter = true) data class AutomationActionResponse(val ok:Boolean=false,val error:String?=null,val event:AutomationEvent?=null)
